...
const validateCard = (array) => {
  ...
  for (let i=0; i<array.length; i++) {
    if (i === 0 || i % 2 === 0) {
      reversedArray[i] = (reversedArray[i]);
    } else if (reversedArray[i]*2 > 9){
       reversedArray[i] = (reversedArray[i]*2-9)
    } else {
       reversedArray[i] = (reversedArray[i]*2)
    }}

  sumOfArray = reversedArray.reduce((a,b) => a+b);

  luhnNumber = sumOfArray % 10 === 0
  return luhnNumber

}

const findInvalidCards = (nestedArray) => {
  return nestedArray.filter(element => !validateCard(element))
}
  ...
  invalidCards.map((card,index) => {
    switch (card[0]) {
      ...
      case 6:
        ...
        break;
      default:
        console.log(`Company not found: starting with ${card[0]} at index ${index}. Card number: ${(card)}`)
        break;
    }
  })
